...
function ajax(options) {
	...
	var url = options.url, // URL
		method = (options.type || "get").toUpperCase(), // 请求方式
...
// 事件委派
function delegate(element, type, className, callback) {
	function cb(e) {
		e = e || event;
		var src = e.target || e.srcElement;
		if (src !== element && src.className === className) {
			var ncb = callback.bind(src);
			ncb(e);
		}
	}
